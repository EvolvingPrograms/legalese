// Phase-1 second rail: a re-implementation of legalese's marker
// substitution using markdsl primitives instead of a hand-rolled
// regex+switch walker. Behavior is intended to match `./substitute.ts`
// exactly so we can swap the implementations at runtime via a flag
// (Phase 2) without changing test output.
//
// The five marker forms legalese supports become five handler
// registrations on a markdsl `MarkerRegistry`:
//
//   ^  small-caps           — pure literal pass-through with HTML span
...
//
// Each handler reaches for markdsl's parse / lookup / compose
// utilities; per-marker policy (BLANK fallback, comma-compose,
// required+missing, trailing-dot swallow) stays in legalese.
 ...
 *  policies, via the markdsl walker. Plus the legalese-specific
 *  trailing-dot swallow that markdsl's substituteMarkers doesn't apply
 *  by default (we layer it on top). */
export function substituteMarkersViaMarkdsl(
  body: string,
  opts: { schema?: Schema; values?: Record<string, unknown> } = {},
): string {
  // Trailing-dot swallow lives per-handler (only `{{=key}}` needs it
  // in legacy semantics), so the walker output here is final.
