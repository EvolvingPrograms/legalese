...
test('title `{{=COMPANY}}` uppercases the substituted value', async () => {
  const { convertMarkdown } = await import('@/md/convert');
  const path = await import('node:path');
  const { ROOT, readDocumentXml, plain } = await import('./_helpers');
...
test('title `{{=COMPANY}}` prefers value over schema.def; uppercases value', async () => {
  const { convertMarkdown } = await import('@/md/convert');
  const path = await import('node:path');
  const { ROOT, readDocumentXml, plain } = await import('./_helpers');
  ...
  expect(body).not.toContain('Override Co.');  // original case absent
});
...
test('::: {.indent} ... ::: applies first-line indent to each child paragraph', async () => {
  ...
  expect(para).toMatch(/<w:ind\s+w:firstLine="540"/);
});
...
test('document-level `indent: true` applies first-line indent to every body paragraph', async () => {
  ...
  const matches = xml.match(/<w:ind\s+w:firstLine="540"/g) ?? [];
...
test('document-level indent omitted (default) does NOT apply first-line indent', async () => {
  ...
  expect(xml).not.toMatch(/<w:ind\s+w:firstLine="540"/);
});
